...
BEGIN
    ...
    -- Retrieve employee details based on the provided EmployeeID
    SELECT @EmployeeName = EmployeeName,
           @Salary = Salary
    FROM Employees
    WHERE EmployeeID = @EmployeeID

    -- Calculate HRA (House Rent Allowance), DA (Dearness Allowance), PF (Provident Fund), and IT (Income Tax) based on salary
    SET @HRA = @Salary * 0.10
    SET @DA = @Salary * 0.20
    SET @PF = @Salary * 0.08
    SET @IT = @Salary * 0.05
...
-- Execute the GeneratePayslip stored procedure for EmployeeID
EXEC GeneratePayslip @EmployeeID = 3;
=======
create database Assignment_5
use Assignment_5
